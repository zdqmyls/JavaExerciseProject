...
public class RecordsDao {
    ...
    public boolean addAccountRecords(Session session, Records records) {
        ...
        try {
            ...
            //发送SQL语句
            String sql = "insert into tb_records" +
                    "(uId,rType,rMoney,rClassification,rMemo,rDate)" + "values(" +
                    session.getUsers().getUserId() + ",'" +
                    records.getRecordType() + "'," +
                    records.getRecordMoney() + ",'" +
                    records.getRecordClassification() + "','" +
                    records.getRecordMemo() + "','" +
    ...
    public Records checkAccountItem(Records records) {
        ...
        try {
            ...
            //发送SQL语句
            String sql =
                    "select rId,rType,rMoney,rClassification,rMemo,rDate from tb_records where rId="+records.getRecordId() +" " + " and uId="+new Session().getUsers().getUserId()+";";
    ...
    public boolean alterAccountItem(Records records) {
        ...
        try {
            ...
            stmt = conn.createStatement();
            //发送SQL语句
//            String sql="update ACCOUNTS set ATYPE='"+accountItem.getType()+"',"+"AMONEY="+accountItem.getMoney()+",
//            "+"ACLASSIFICATION='"+accountItem.getClassification()+"',"+"AMEMO='"+accountItem.getMemo()+"',
//            "+"ADATE='"+accountItem.getDate()+"'"+" where AID="+accountItem.getId()+";";
     ...
     * @return float 返回求和结果
     */
    public float getAccountTotal(String sql) {
        Connection conn = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
    ...
    public List getAllTableDataByUser(Session session) {
        ...
        try {
            ...
            //发送SQL语句
            String sql =
                    "select rId,rType,rMoney,rClassification,rMemo,rDate from tb_records where uId=" + session.getUsers().getUserId() + ";";
            rs = stmt.executeQuery(sql);
            while (rs.next()) {
