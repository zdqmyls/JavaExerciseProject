...
public class PieChartFrameController {
    ...
    public void initComboBox() {
        publicTools.public_addComboBoxItems(pieChart_comboBox, new String[]{"今天", "昨天", "最近1周收入", "最近1周支出", "最近1" +
                "年支出（按月份）", "最近1年收入（按月份）", "最近1年支出（按季度）", "最近1年收入（按季度）"});
    }
    ...
    public void pieChart_comboBoxEvent(ActionEvent event) {
        ...
        if (selectedCoboboxItem.equals("今天")) {
            chartTools.public_setDayPieChartData(new Session().getUsers(), pieChart, new Date(), new Date());
        } else if (selectedCoboboxItem.equals("昨天")) {
            chartTools.public_setDayPieChartData(new Session().getUsers(), pieChart, dateTools.getYesterdayDate(),
                    dateTools.getYesterdayDate());
        } else if (selectedCoboboxItem.equals("最近1周支出")) {
            chartTools.public_setWeekPieChartData(new Session().getUsers(), 7, pieChart, "支出");
        } else if (selectedCoboboxItem.equals("最近1周收入")) {
            chartTools.public_setWeekPieChartData(new Session().getUsers(), 7, pieChart, "收入");
        } else if (selectedCoboboxItem.equals("最近1年支出（按月份）")) {
            Calendar calendar = Calendar.getInstance();
            int month = calendar.get(Calendar.MONDAY) + 1;
            chartTools.public_setMonthPieChartData(new Session().getUsers(), month, pieChart, "支出");
        } else if (selectedCoboboxItem.equals("最近1年收入（按月份）")) {
            Calendar calendar = Calendar.getInstance();
            int month = calendar.get(Calendar.MONDAY) + 1;
            chartTools.public_setMonthPieChartData(new Session().getUsers(), month, pieChart, "收入");
        } else if (selectedCoboboxItem.equals("最近1年支出（按季度）")) {
            chartTools.public_setSeasonPieChartData(new Session().getUsers(), pieChart, "支出");
        } else if (selectedCoboboxItem.equals("最近1年收入（按季度）")) {
            chartTools.public_setSeasonPieChartData(new Session().getUsers(), pieChart, "收入");
        }
