...
public class classificationCheckFrameController {
    ...
    private TableView<TableData> input_classification_tableView; // Value injected by FXMLLoader
    ...
    private TableView<TableData> output_classification_tableView; // Value injected by FXMLLoader
    ...
    public void deleteContextMenuEvent(ActionEvent event) {
        publicTools.public_do_deleteContextMenu_Event(input_classification_tableView);
        publicTools.public_do_deleteContextMenu_Event(output_classification_tableView);
    }

    // ”添加“右键菜单的事件方法
    public void addContextMenuEvent(ActionEvent event) {
        publicTools.public_do_addContextMenu_Event(input_classification_tableView);
        publicTools.public_do_addContextMenu_Event(output_classification_tableView);
    }

    // ”修改“右键菜单的事件方法
    public void alterContextMenuEvent(ActionEvent event) {
        publicTools.public_do_alterContextMenu_Event(input_classification_tableView);
        publicTools.public_do_alterContextMenu_Event(output_classification_tableView);
    }

    // ”支出“选项卡中的下拉菜单的事件方法
    public void outputClassificationComboBoxEvent(ActionEvent event) {
        //只处理选中的状态
        String selectedCoboboxItem =
                (String) outputClassificationComboBox.getSelectionModel().selectedItemProperty().getValue();
        String sql =
                "select rId,rType,rMoney,rClassification,rMemo,rDate from tb_records where rClassification='" + selectedCoboboxItem + "' and uId=" + new Session().getUsers().getUserId() + ";";
    ...
    public void inputClassificationComboBoxEvent(ActionEvent event) {
        //只处理选中的状态
        String selectedCoboboxItem =
                (String) inputClassificationComboBox.getSelectionModel().selectedItemProperty().getValue();
        String sql =
                "select rId,rType,rMoney,rClassification,rMemo,rDate from tb_records where rClassification='" + selectedCoboboxItem + "' and uId=" + new Session().getUsers().getUserId() + ";";
    ...
    public void initialize() throws IOException {
        publicTools.public_addComboBoxItems(inputClassificationComboBox,
                publicTools.public_getInputClassification());
        publicTools.public_addComboBoxItems(outputClassificationComboBox,
